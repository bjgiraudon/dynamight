...
def repDyn3(X, t, payMtx):
    '''Computes the replicator dynamics for a symmetric 2P3S game'''
    x1, x2 = X
...
def repDyn3Speed(x1, x2, payMtx):
    '''Computes the replicator dynamics for a symmetric 2P3S game | adapted for speed grids'''
    Pbar = w3(x1, x2, x1, x2, payMtx)
    return np.array([x1*(w3(1, 0, x1, x2, payMtx) - Pbar), x2*(w3(0, 1, x1, x2, payMtx) - Pbar)])

def repDyn3Rev(X, t, payMtx):
    '''Computes the opposite replicator dynamics for a symmetric 2P3S game'''
    x1, x2 = X
    Pbar = w3(x1, x2, x1, x2, payMtx)
    return np.array([-x1*(w3(1, 0, x1, x2, payMtx) - Pbar), -x2*(w3(0, 1, x1, x2, payMtx) - Pbar)])
